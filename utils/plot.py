...
class GAN():
    ...
    def data(X:"tensor", **plot_kwargs:dict) -> None:
        ...
        image_with_labels(X, labels=labels, label_prefix="Image:",title="Training input/Real images",**plot_kwargs)
        #image_with_labels(Y, title="Target",**plot_kwargs)
    @staticmethod
    def frame_data(X:"tensor", Y:"tensor"=None, **plot_kwargs:dict):
        X = X.permute(0,2,3,1)
        Y = Y.permute(0,2,3,1)
        X = GAN.rescale(X)
        Y = GAN.rescale(Y)
        image_with_labels(X, title="First Frames",**plot_kwargs)
        image_with_labels(Y, title="Last Frames",**plot_kwargs)
    @staticmethod
    def rescale(X):
        X = X - torch.min(X) 
        X = X / torch.max(X)
...
def image_with_labels(data:"tensor", labels:"tensor"=None, title:str=None, nimages:int=10, nrows:int=2, fig_dimension=1,title_size=10, label_prefix="Label: ", cmap=None) -> None:
